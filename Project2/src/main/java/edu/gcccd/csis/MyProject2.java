...
public class MyProject2 implements Project2 {
    ...
    /*
    * Moreover, you need to provide a detailed estimate for how often on average ANY iterators' next()
    * method gets called (depending on the value of L), when addition(Iterator<NodeList<Integer>> iterator)
    * gets called.
    *
    *The number of times iterators' next() methods gets called is
    *               L * (5 * ((E + 1) / 2) )
    *      L is the number Nodelists
    *      E is the number of elements possible in a Nodelist
    *      Both can be substituted for N for number of times next() is called
    *
    *   Rewritten :  5/2 ( N^2  + N )
